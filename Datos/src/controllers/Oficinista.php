...
public function read( Request $request, Response $response, array $args ){
    $sql = "SELECT * FROM Oficinistas ";

    if (isset($args['id'])) {
    $sql .= " WHERE id_oficinista = :id ";
...
public function create( Request $request, Response $response, array $args ){
        ...
        $sql = "INSERT INTO Oficinistas (cedula, nombre, email) VALUES (:cedula, :nombre, :email)";
        $con = $this->conteiner->get('conexion');
       ...
       $query = $con->prepare($sql);
        ...
        $rol = 2; // rol para oficinistas según Roles enum del frontend
        $query = $con->prepare($sql);
...
public function update( Request $request, Response $response, array $args ){
       ...
       try{
        ...
        $q = $con->prepare("SELECT cedula FROM Oficinistas WHERE id_oficinista = :id");
        $q->execute(['id' => $id]);
        $current = $q->fetch();
        $oldCedula = $current ? $current->cedula : null;

       $sql = "UPDATE Oficinistas SET cedula = :cedula, nombre = :nombre, email = :email WHERE id_oficinista = :id";
       $query = $con->prepare($sql);
 ...
 public function delete(Request $request, Response $response, array $args){
        ...
        try{
            ...
            $query = $con->prepare("DELETE FROM Oficinistas WHERE id_oficinista = :id");
            $query ->execute(['id' => $id]);
...
public function filter(Request $request, Response $response, array $args){
        ...
        $sql = "SELECT *, COUNT(*) OVER() as total FROM Oficinistas WHERE 1=1";
        $params = [];
