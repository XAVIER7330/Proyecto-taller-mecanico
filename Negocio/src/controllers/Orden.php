...
namespace App\controllers;
 ...
 class Orden extends ServicioCurl {
    private $conteiner;
    Private const ENDPOINT = "/orden";
...
public function cambiarEstado( Request $request, Response $response, array $args ){
        ...
        $body = json_decode($request->getBody());
        $id_orden = $args['id'];
        $uri = self::ENDPOINT."/estado/".$id_orden;

        $resp = $this->ejecutar($uri, 'PATCH', json_encode($body));
        $status = $resp['status'];
        $resp = $resp['body'];
